...
/**
 * desc: 格式化制定月份 价格区间为单位的  租房和合租房详情页的 pv数据
 ...
 *  		value:
 * */
public class PageViewFormat4PriceSectionOfMouth {
	...
	public static class M1 extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException,InterruptedException {
			try{
				...
				if(fields[4].indexOf("_2")<0||fields[7].indexOf("A")>0){
					return ; 
				}
				...
				Long kLon=KeyUtil.getKey( k.split(FS), datetime , Table.house_pv_price);
...
//		String endDate = otherArgs[0].split(",").length == 2 ? otherArgs[0].split(",")[1] : startDate;
		...
		String baseOutPath = "/dsap/resultdata/sa/zufang_pv_pricesection_res";
    	...
    	for (int y = 0; y < ms.length;y++) {
    		...
    		Job job = new Job(conf, "PageViewFormat4PriceSectionOfMouth");
    		job.setJarByClass(PageViewFormat4PriceSectionOfMouth.class);
