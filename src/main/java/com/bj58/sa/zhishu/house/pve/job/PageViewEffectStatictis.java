...
public class PageViewEffectStatictis {
	...
	public static class R1 extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException,
			...
			oVal.set( pv + FS+size+FS +f1);
			context.write(oKey, oVal);
			
			double temp=f1*WLRandomUtil.getRandom(DATE.substring(0,6));
			BigDecimal   b1   =   new   BigDecimal(temp);  
			Double   f2   =   b1.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();  
			
			oKey.set(outKey+ FS + DATE+FS +"WL" );
			oVal.set( pv + FS+size+FS +f2);
			context.write(oKey, oVal);
		}
	}
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException,
		...
		String baseOutPath = "/dsap/resultdata/sa/zufang_pve";
